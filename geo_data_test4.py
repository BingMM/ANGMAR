...
def create_hexagonal_grid(center_x, center_y, spacing_km, n_rings, transformer):
    """
    Create a hexagonal grid of magnetometer positions with equal spacing
    ...
    # Hexagonal grid has 6 fundamental directions
    # For a proper hexagonal grid, we use vectors at 60-degree intervals
    hex_angles = np.array([0, 60, 120, 180, 240, 300]) * np.pi / 180
    
    # Add center point
    lon, lat = transformer.transform(center_x, center_y, direction='INVERSE')
    proposed_sites.append((center_x, center_y, lon, lat))
    
    # For a hexagonal grid, we need to consider all points that can be reached
    # by combinations of the two base vectors
    # Base vectors for hexagonal grid (60 degrees apart)
    v1 = np.array([1, 0]) * spacing_m
    v2 = np.array([np.cos(np.pi/3), np.sin(np.pi/3)]) * spacing_m  # 60 degrees
    
    # Generate all grid points within n_rings
    for i in range(-n_rings, n_rings + 1):
        for j in range(-n_rings, n_rings + 1):
            # Skip if outside the desired range
            if abs(i) + abs(j) > n_rings or (i > 0 and j > 0 and i + j > n_rings):
                continue
            if i < 0 and j < 0 and abs(i + j) > n_rings:
                continue
                
            # Calculate position using hexagonal grid vectors
            x = center_x + i * v1[0] + j * v2[0]
            y = center_y + i * v1[1] + j * v2[1]
