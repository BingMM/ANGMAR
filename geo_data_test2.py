...
def plot_magnetometer_network_osm(magnetometer_coords, center_lat, center_lon, radius_km=1500, 
    ...
    # Plot radius circle around grid center
    circle = plt.Circle((grid_center_x, grid_center_y), radius_km * 1000, fill=False, 
                       edgecolor='darkred', linewidth=3, linestyle='--', alpha=0.7)
    ax.add_patch(circle)
    
    # Generate hexagonal grid for proposed magnetometers
    spacing_m = 100000  # 150 km in meters
    proposed_sites = []
    
    # Calculate grid extent
    n_rings = int(radius_km / 100) + 1
    
    for ring in range(n_rings + 1):
        if ring == 0:
            # Center point of grid
            proposed_sites.append((grid_center_x, grid_center_y, grid_center_lon, grid_center_lat))
        else:
            # Points in hexagonal rings
            n_points = 6 * ring
            for i in range(n_points):
                angle = 2 * np.pi * i / n_points
                x = grid_center_x + ring * spacing_m * np.cos(angle)
                y = grid_center_y + ring * spacing_m * np.sin(angle)
    ...
    ax.set_title('Proposed Magnetometer Network Around EISCAT 3D\n' + 
                f'100 km Grid Spacing - {radius_km} km Radius Coverage', 
                fontsize=18, fontweight='bold', pad=20)
    
    # Set axis limits with margin based on grid center
    margin = radius_km * 1000 * 0.15
    ax.set_xlim(grid_center_x - radius_km*1000 - margin, grid_center_x + radius_km*1000 + margin)
    ax.set_ylim(grid_center_y - radius_km*1000 - margin, grid_center_y + radius_km*1000 + margin)
